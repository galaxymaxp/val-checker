-- A disconnect must not mint a second automatic storefront allowance for the
-- same user on the same UTC store date. Historical runs intentionally survive
-- connection deletion, so count them against the number of accounts currently
-- held. Adding a genuinely separate account still increases capacity by one.
begin;
...
comment on function public.claim_riot_daily_run(uuid, uuid, uuid, uuid) is
  'Locks the exact shared lease, serially claims at most one automatic run per current account per UTC store date, safely recovers five-minute-stale pre-attempt claims, and preserves spent capacity across reconnects.';
