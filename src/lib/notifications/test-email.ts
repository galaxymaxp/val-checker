 ...
 */
/** Valorant Points. Other currencies are promotional. */
...
export async function sendStorefrontTestEmail(
  ...
  // The offer's reward carries the resolved skin uuid directly; the top-level
  // skinUuid is only filled by later catalog enrichment and is usually null.
  const skinUuid =
    offer.skinUuid ??
    offer.rewards?.find(
      (reward: { skinUuid?: string }) => reward.skinUuid,
    )?.skinUuid ??
    null;
  ...
  const rendered = renderStorefrontMatchEmail({
    ...
    imageUrl,
    match: { offers: [], skinUuid: skinUuid ?? "unknown-skin" },
    priceVp,
  });
